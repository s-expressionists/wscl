...
\param{method-combination}---method combination object.
 
\param{environment}---the same as the \keyref{environment} argument
...
\issue{SLOT-VALUE-MISSING-SETF:ADD}
\DefunWithValues {slot-value} {object slot-name} {value}
\endissue{SLOT-VALUE-MISSING-SETF:ADD}
...
of a \macref{make-method}, the arguments are those that
...
must be the symbol \macref{make-method} and the second element must be
...
the first element of which must be the symbol \macref{make-method} and the
second element of which must be a \term{form}.
 
% Added by Moon:
 
Those are the only two places where \macref{make-method} can be used.
The \term{form} used with \macref{make-method} is evaluated in
...
\macref{defmethod}, \macref{call-method}, \macref{define-method-combination}.
...
the \macref{call-method} macro whose first \term{subform} is a method object and
...
\specref{multiple-value-prog1}, and the like, if this optimization is desired.
